...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT id, username, email, password_hash, date_joined FROM User"
            )
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT id, user_id, title, kind, target_value, color_hex, created_at FROM Habit"
            )
            ...
            for habit_data in legacy_habits:
                ...
                if user_id in user_mapping:
                    ...
                    if existing_habit:
                        self.stdout.write(
                            f'Habit {title} already exists, skipping...'
                        )
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT id, user_id, date, name, calories, protein_g, carbs_g, fat_g FROM NutritionEntry"
            )
            ...
            for nutrition_data in legacy_nutrition:
                ...
                if user_id in user_mapping:
                    ...
                    if existing_meal:
                        self.stdout.write(
                            f'Meal {name} on {date} already exists, skipping...'
                        )
                    else:
                        meal = Meal.objects.create(
                            ...
                            date=date,
                            notes=(
                                f"Migrated from legacy: {name} - "
                                f"Calories: {calories}, Protein: {protein_g}g, "
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT id, habit_id, date, duration_minutes, notes, created_at, duration_min FROM Workout"
            )
            ...
            for workout_data in legacy_workouts:
                ...
                if habit_id in habit_mapping:
                    ...
                    if existing_workout:
                        self.stdout.write(
                            f'Workout on {date} already exists, skipping...'
                        )
                    else:
                        # Convert duration to timedelta
                        from datetime import timedelta
                        duration = timedelta(
                            minutes=duration_minutes or duration_min or 30
                        )
